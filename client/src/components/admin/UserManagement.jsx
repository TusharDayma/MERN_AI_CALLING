...
import { UserPlus, Shield, Ban, Trash2, X, ShieldAlert, Users as UsersIcon, AlertCircle, Coins } from 'lucide-react';
...
export default function UserManagement() {
  ...
  const handleUpdateCredits = async (e) => {
    e.preventDefault();
    try {
      await api.patch(`/admin/users/${selectedUser.id}/credits`, { credits: creditsInput });
  ...
  return (
    <DashboardLayout role="ADMIN">
            ...
            <table className="data-table">
              ...
              {users.map((u) => (
                <tr key={u.id} className="hover:bg-surface-raised/60 transition-colors">
                  ...
                  <td className="text-sm font-semibold text-info">
                    {u.credits_balance || 0}
                  </td>
                  ...
                  <td className="pr-5">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button 
                        onClick={() => { setSelectedUser(u); setCreditsInput(u.credits_balance || 0); setIsCreditsModalOpen(true); }}
                        className="btn btn-sm bg-info/10 text-info border border-info/20 hover:bg-info hover:text-white"
      ...
      {isCreditsModalOpen && selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-text-primary/40 backdrop-blur-sm">
          <div className="bg-surface border border-border shadow-xl rounded-2xl p-6 w-full max-w-sm relative">
            <button onClick={() => setIsCreditsModalOpen(false)} className="absolute top-6 right-6 text-text-muted hover:text-text-primary">
              <X className="w-6 h-6" />
            </button>
            <h2 className="text-xl font-bold text-text-primary mb-2">Manage Credits</h2>
            <p className="text-sm text-text-secondary mb-6">Updating credits for {selectedUser.name}</p>
            <form onSubmit={handleUpdateCredits} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Credits Balance</label>
                <input required type="number" min="0" value={creditsInput} onChange={e => setCreditsInput(e.target.value)} className="w-full px-4 py-2 bg-surface border border-border rounded-lg text-text-primary" />
              </div>
              <button type="submit" className="btn-primary w-full mt-4">Save Credits</button>
            </form>
          </div>
        </div>
      )}
